...
public class Main extends Frame implements WindowListener, MineSweeperGUI {
  ...
  public String getTileLabel(int x, int y) {
    return this.tileTable[y][x].getLabel();
  }

  @Override
  public void setDefaultColor(int y, int x){
    this.tileTable[x][y].setBackground(Color.LIGHT_GRAY);
  }

  @Override
  public void setFlagColor(int y, int x) {
    this.tileTable[x][y].setForeground(Color.BLUE);
  }

  @Override
  public void resetFlagColor(int y, int x){
    this.tileTable[x][y].setForeground(Color.black);
  }
  ...
  public void setLoseColor() {
    for(int i = 0; i < ms.getHeight(); i++) {
      for(int j = 0; j < ms.getWidth(); j++) {
        this.tileTable[j][i].setBackground(Color.RED);
      }
    }
  }
  ...
  public void setWinColor() {
    for(int i = 0; i < ms.getHeight(); i++) {
      for(int j = 0; j < ms.getWidth(); j++) {
        this.tileTable[j][i].setBackground(Color.cyan);
      }
    }
  }
